...
CLUSTER_NAME=${CLUSTER_NAME:-"mesh1"}
METALLB_VERSION=${METALLB_VERSION:-"v0.13.10"}
KIND_NODE_TAG=${KIND_NODE_TAG:-"v1.28.0"}
KUBECONFIG=${KUBECONFIG:-"${HOME}/.kube/kind-config-${CLUSTER_NAME}"}
...
if [[ -z "${KIND_NODE_TAG}" ]]; then
  ...
  kind create cluster --image "kindest/node:${KIND_NODE_TAG}" --name "${CLUSTER_NAME}" --kubeconfig "${KUBECONFIG}"
fi

## Install MetalLB.
kubectl apply -f https://raw.githubusercontent.com/metallb/metallb/"${METALLB_VERSION}"/config/manifests/metallb-native.yaml --kubeconfig "${KUBECONFIG}"
...
kubectl rollout status -n metallb-system deployment/controller --timeout 5m --kubeconfig "${KUBECONFIG}"
kubectl rollout status -n metallb-system daemonset/speaker --timeout 5m --kubeconfig "${KUBECONFIG}"
